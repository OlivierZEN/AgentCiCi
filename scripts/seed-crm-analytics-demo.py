...
def verify_batch_old_values(
    ...
    for record in batch:
        ...
        if not record_id or record_id in duplicate_ids or record_id not in current_by_id:
            raise RuntimeError(
                f"{operation} update aborted: live record lookup mismatch for "
                f"{spec.object_api} record {record_label} before batch write"
            )
        changes = record.get("changes")
        if not isinstance(changes, dict) or not changes:
            raise RuntimeError(
                f"{operation} update aborted: invalid changes for "
                f"{spec.object_api} record {record_label} before batch write"
            )
        ...
        for field, change in changes.items():
            if field not in allowed_fields or not isinstance(change, dict) or "old" not in change:
                raise RuntimeError(
                    f"{operation} update aborted: invalid change field for "
                    f"{spec.object_api} record {record_label} before batch write"
                )
            if reference_id(current.get(field)) != reference_id(change.get("old")):
                raise RuntimeError(
                    f"{operation} update aborted: live drift detected for "
...
def apply_update_plan(
    ...
    project: Path,
    cloudcc_cli: Path = CLOUDCC,
    verify_live_state_for: str | None = None,
) -> dict[str, int]:
    ...
    for spec in MIGRATION_SPECS:
        ...
        for batch in chunks(object_records):
            if verify_live_state_for:
                verify_batch_old_values(
                    batch,
                    spec,
                    project,
                    cloudcc_cli,
                    verify_live_state_for,
                )
            payload = [
                {
        ...
        if args.execute:
            ...
            if args.rollback_from:
                ...
                report["writeStats"] = apply_update_plan(
                    ...
                    args.cloudcc_cli,
                    verify_live_state_for="rollback",
                )
