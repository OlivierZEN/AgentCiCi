...
public class AgentMemoryCandidateController {
    ...
    @GetMapping public ApiResponse<List<CandidateView>> list(@PathVariable String agentId) { requireManage(agentId); return ApiResponse.ok(candidates.list(TenantContext.requireCompanyId(), agentId).stream().map(CandidateView::from).toList()); }
    @PostMapping("/{candidateId}/approve") public ApiResponse<RecordView> approve(@PathVariable String agentId, @PathVariable Long candidateId, @Valid @RequestBody ReviewRequest request) { requireManage(agentId); var record=candidates.approve(TenantContext.requireCompanyId(), agentId, candidateId, actor(), request.reason()); return ApiResponse.ok(RecordView.from(record)); }
    @PostMapping("/{candidateId}/reject") public ApiResponse<Void> reject(@PathVariable String agentId, @PathVariable Long candidateId, @Valid @RequestBody ReviewRequest request) { requireManage(agentId); candidates.reject(TenantContext.requireCompanyId(), agentId, candidateId, actor(), request.reason()); return ApiResponse.ok(null); }
    @GetMapping("/records") public ApiResponse<List<RecordView>> records(@PathVariable String agentId) { requireManage(agentId); return ApiResponse.ok(records.findByCompanyIdAndAgentIdOrderByUpdatedAtDesc(TenantContext.requireCompanyId(), agentId).stream().map(RecordView::from).toList()); }
    @PostMapping("/records/{recordId}/revoke") public ApiResponse<java.util.Map<String, Object>> revoke(@PathVariable String agentId, @PathVariable Long recordId, @Valid @RequestBody ReviewRequest request) { requireManage(agentId); boolean vectorRemoved=lifecycle.revokeRecord(TenantContext.requireCompanyId(), agentId, recordId, actor(), request.reason()); return ApiResponse.ok(java.util.Map.of("recordId", recordId, "vectorRemoved", vectorRemoved)); }
    @PostMapping("/subjects/delete") public ApiResponse<MemoryLifecycleService.DeletionResult> deleteSubject(@PathVariable String agentId, @Valid @RequestBody SubjectDeleteRequest request) { requireManage(agentId); return ApiResponse.ok(lifecycle.deleteSubject(TenantContext.requireCompanyId(), request.applicationCode(), request.subjectType(), request.externalSubjectRef(), agentId, actor(), request.reason())); }
    private void requireManage(String agentId) { access.require(TenantContext.requireCompanyId(), actor(), TenantContext.getRoles(), agentId, AgentPermission.MANAGE); }
