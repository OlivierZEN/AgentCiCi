...
public class MemoryCandidateGovernanceService {
    ...
    @Transactional public MemoryCandidateEntity submit(ExternalMemoryContextService.ExternalMemoryContext context, String agentId, ExternalMemoryContextService.MemoryWriteCommand command) {
        ...
        return candidates.save(new MemoryCandidateEntity(context.companyId(), agentId.trim(), subject.getId(), command.scope(), command.scopeKey(), command.memoryType(), command.content(), command.sensitivity(), command.confidence()==null?BigDecimal.ONE:command.confidence(), command.validFrom()==null?Instant.now():command.validFrom(), command.validTo(), command.sourceType(), command.sourceRefsJson()==null?"[]":command.sourceRefsJson()));
    }
    @Transactional public MemoryRecordEntity approve(String companyId, String agentId, Long candidateId, String reviewer, String reason) {
        MemoryCandidateEntity candidate=requireCandidate(companyId, agentId, candidateId);
        ...
        MemoryRecordEntity record=records.save(new MemoryRecordEntity(companyId, candidate.getSubjectId(), candidate.getScope(), candidate.getScopeKey(), candidate.getMemoryType(), candidate.getContent(), "ACTIVE", candidate.getSensitivity(), candidate.getConfidence(), candidate.getValidFrom(), candidate.getValidTo(), candidate.getSourceType(), candidate.getSourceRefsJson()));
        record.assignAgent(agentId); records.save(record);
        semanticRetrieval.index(record);
        audit.log(companyId, reviewer, "agent.memory.candidate.approve", "agent=" + agentId + ",candidateId=" + candidateId);
        return record;
    }
    @Transactional public void reject(String companyId, String agentId, Long candidateId, String reviewer, String reason) { MemoryCandidateEntity c=requireCandidate(companyId, agentId, candidateId); if (!"PENDING".equals(c.getStatus())) throw new IllegalStateException("memory candidate is not pending"); c.review("REJECTED", reviewer, reason); candidates.save(c); audit.log(companyId, reviewer, "agent.memory.candidate.reject", "agent=" + agentId + ",candidateId=" + candidateId); }
    @Transactional(readOnly=true) public java.util.List<MemoryCandidateEntity> list(String companyId, String agentId) { return candidates.findByCompanyIdAndAgentIdOrderByUpdatedAtDesc(companyId, agentId); }
    private MemoryCandidateEntity requireCandidate(String companyId, String agentId, Long candidateId) { return candidates.findByIdAndCompanyIdAndAgentId(candidateId, companyId, agentId).orElseThrow(() -> new IllegalArgumentException("memory candidate not found")); }
}
