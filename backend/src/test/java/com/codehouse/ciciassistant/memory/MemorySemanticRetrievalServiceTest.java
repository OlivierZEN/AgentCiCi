...
class MemorySemanticRetrievalServiceTest {
    @Test void returnsOnlyRecordsAlreadyAuthorizedByTheRelationalContext() throws Exception {
        ...
        when(embeddings.embed("query")).thenReturn(List.of(1f));
    ...
    @Test void redactsBeforeEmbeddingAndDoesNotPropagateIndexFailure() throws Exception {
        ...
        when(embeddings.embed(contains("[email]"))).thenReturn(List.of(1f)); when(vectors.upsert(any())).thenThrow(new IllegalStateException("offline"));
        new MemorySemanticRetrievalService(embeddings, vectors, fragments, mock(MemoryRecordRepository.class), mock(ExternalMemoryContextService.class), new SecurityRedactionService()).index(record);
        verify(embeddings).embed(contains("[email]")); verify(fragments, never()).save(any());
    }
