...
- 业务本体工作台：桌面端采用“业务目录—语义画布—业务检查器”三栏结构；画布节点、数据映射行和 AI 提案差异均使用业务名称优先、必要结构线分隔的紧凑语汇，不向业务人员暴露连接密钥、草稿内部 JSON 或实现标识。节点移动需同时支持指针拖动和方向键，AI 提案必须在明确审阅后才可应用，发布始终经过独立人工确认弹窗。草稿存在本地改动或修订冲突时，返回列表、侧栏跳转、退出后台与刷新页面都必须先确认；冲突状态允许先复制脱敏后的本地草稿。
