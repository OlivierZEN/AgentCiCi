---
updated_at: 2026-04-24T09:52:33Z
status: active
---
...
- ISSUE-2026-04-24-cici-session-history-not-injected:
  ...
  - Verified root cause: `ChatOrchestratorService.buildInitialMessages(...)` 当前只注入 system prompt、当前用户问题、RAG 内容与长期用户记忆，没有回灌同一 `sessionId` 的历史消息；同时系统缺少会话级执行状态层来表达“已确认动作 / 暂缓动作 / 当前对象 / 缺失字段”。
  - Evidence: code inspection on 2026-04-24 of `ChatOrchestratorService`, `ChatMessageEntity`, `ChatMessageRepository`, `UserMemoryService`, `ToolOrchestratorService`, and frontend `AssistantApp` session reuse flow.
  - Status: open (P0，直接影响思思多轮协作的稳定性；设计方案已落在 `docs/specs/FEAT-002-cici-session-continuity-and-state.md`)。
...
- ISSUE-2026-04-23-mcp-smoke-blocked-by-admin-auth-scope:
  ...
  - Verified root cause (inferred): `13900009999` 在本轮会话触发了 `SMS request too frequent`，无法继续发码登录；可登录的 `13800138111` 虽 JWT `roles=["ORG_ADMIN"]`，但调用 `/mcp-servers` / `POST /mcp-servers` 返回 `需要组织管理员权限`，且读取到的 MCP 列表为空，导致无法完成“未同步 -> 刷新成功 -> 刷新失败保留旧快照”全链路验证。
  - Evidence: API smoke commands run on 2026-04-23 (`/auth/sms/send`, `/auth/sms/login`, `/mcp-servers`, `/mcp-servers/{id}/tools`, `/mcp-servers/{id}/discover`, `/mcp-servers POST`) with returned JSON captured in `.claw/test-report.md`.
  - Status: open (P1，阻塞本轮 MCP 缓存功能的真实运行态验收；不影响后端/前端构建通过)。

- ISSUE-2026-04-21-agent-runtime-not-bound-to-published-workflow:
  - Symptom: Agent Builder 可以保存、编译、发布版本，但当前线上聊天链路并不会按已发布版本执行对应 workflow。
  - Verified root cause: `publishVersion(...)` 仅更新 `agent_definition.published_version_id` 与版本 `publish_status`；聊天运行时仍通过 `SkillResolverService` 解析 skills / tools / kb，并未读取 `agent_workflow_version.workflow_code`、`workflow_manifest` 或 `workflow_preview`。
  - Evidence: code inspection on 2026-04-21 of `AgentDefinitionService.publishVersion(...)`, `ChatOrchestratorService.chat/chatStream(...)`, `SkillResolverService.resolve(...)`.
  - Status: open (P0，缺少“发布版本 -> 运行时生效”的关键闭环)。
- ISSUE-2026-04-21-agent-debug-still-simulated:
  - Symptom: Agent Builder 中“试运行”只能高亮前端模拟路径，无法给出真实后端执行 trace、工具调用或命中分支证据。
  - Verified root cause: frontend `runDebug()` 调用 `simulateDebugTrace(...)`；仓库实现中未发现 `POST /agents/{agentId}/debug` 后端接口。
  - Evidence: code inspection on 2026-04-21 of `frontend/src/assistant/AgentBuilderShell.tsx` and backend route search.
  - Status: open (P0，阻塞“可调试的自动化编排”验收)。
...
- ISSUE-2026-04-08-cloudcc-token-invalid-credential:
  ...
  - Verified root cause: direct token request to resolved gateway (`/api/cauth/token`) returns `result=false`, `returnInfo=Please check your username and password.`
  - Evidence: DB has persisted `cc_username` + `cc_safetymark` for user `Owen`, and integration app `cloudcc_crm` is enabled with org/client/secret config.
