...
- Resolution: TASK-335 增加规范 `text` 字段解析并保留纯文本、`content`、`delta` 兼容；实现 `d47afb41c66d` 进入本地 main，本地 backend/frontend 从该提交构建为 `2.8.67-dev.d47afb4`。
- Verification: 聚焦 4 项、前端全量 58 文件/318 项、production build、域名与 diff 门禁通过；官网用截图原问题返回 678 字正文，消息历史完整，Trace `COMPLETED/model_call_count=2`，浏览器 console 0。
- Status: resolved locally by TASK-335 on 2026-08-28; remote/UAT/production unchanged.
...
- ISSUE-2026-08-31-web-widget-buffered-stream:
  ...
  - Resolution: TASK-347 `19080005` 实现无工具意图 DIRECT、安全增量输出、明确 buffered 降级、阶段提示和 TTFT/首 delta 观测，并删除固定字数/sleep 模拟分片。
  - Verification: 本地 website Trace 为单模型、零工具、`streaming`，供应商/客户端首 delta `8.246s / 8.320s`、总耗时 `10.385s`；浏览器首段早于完成约 `3.867s`，期间多次可见增长。
  - Status: resolved locally; UAT 仍为 `2.8.68-beta.3 / a5bbb114` 且未包含修复，生产未修改。
...
- ISSUE-2026-08-17-devautopilot-keyword-routing:
  ...
  - Resolution progress: 首轮实现 `e083fb87` 的真实验收又暴露思考模式不支持命名 `tool_choice`，导致请求在模型推理前以 0 Token 失败；`f121d20c` / `cc4312ed` 已采用思考兼容唯一工具判定、非思考协议重试、动作相关字段校验和结构化字段别名规范化。真实 Provider 探测已覆盖隐含创建与否定解释。
  - Status: resolved locally in `2.8.61-dev.cc4312e`; page dialogue acceptance pending; UAT/production unchanged.
