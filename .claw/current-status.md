...
next_action: "完成 AgentCiCi 顶层租户 org_id→company_id 破坏性统一的 fresh PostgreSQL 回归、跨项目契约记录与提交；不得在未安排维护窗口前发布。"
read_next:
  goals: false
  decisions: false
  issue_list: true
...
- TASK-242 / FEAT-135：用户已确认以 `company_id` 统一 AgentCiCi 与 Semattice 的顶层企业身份，`organization_id` 只为未来公司内部组织架构保留。实现使用 V94 正向迁移将当前 131 个顶层 `org_id` 列和根表/生命周期表物理改名为 `company_id`/`company*`，不重写既有 `org...` ID 值；旧 JSON、旧 Header 与旧 JWT claim 不保留兼容，必须 fail closed。此变更要求维护窗口，旧二进制不能连接 V94 schema。代码、前端构建和 fresh PostgreSQL 迁移已完成首轮验证，正在收口最终回归与 Semattice 契约文档；尚未为本项发布生产。
