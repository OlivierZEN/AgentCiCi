...
next_action: "等待 TASK-249 的合并或生产发布授权；发布后以受权组织管理员复核组织简档。"
read_next:
  goals: false
  decisions: false
  issue_list: true
...
- TASK-249 / FEAT-142：已完成本地修复并进入 review。根因是生产 Nginx 和本地 Vite 仍代理旧 `/admin/organization/...`，而当前 `GET /admin/company/profile` 落入 SPA `index.html`：生产匿名请求返回 `200 text/html`，不是后端鉴权 JSON。已在 Vite、HTTP Nginx 与 HTTPS Nginx 中加入精确 `admin/company/profile` 代理，保留旧组织路径兼容；Nginx 语法、Compose、前端构建和 diff 检查通过。当前生产 V96 的组织简档统计相关表和 `company_id` 字段已只读核实存在；未获授权未修改线上 Nginx、镜像或容器。
