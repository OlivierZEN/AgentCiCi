...
next_action: "Complete final independent review of the V83 provenance fix, then merge and execute the production runbook and acceptance gates."
read_next:
  goals: false
  decisions: false
  issue_list: true
...
- TASK-213 is in progress on `codex/TASK-213-general-ontology-v1`: the domain-neutral backend, V82 persistence, project-delivery/CloudCC reference paths, read-only semantic query and business-first visual workbench are implemented locally. The authorized forward-only V83 now records `MANUAL` or `REFERENCE_PACKAGE` plus non-empty package ID and the SHA-256 of the exact classpath package bytes; admin responses expose that provenance, and reference-package result recovery requires exact source, package ID, fingerprint, business identity and creator. A same-administrator manual workspace with identical metadata is therefore not accepted. V82 remains unchanged and the schema still has 13 ontology tables. TDD, a final fresh PostgreSQL run from zero (39/39), frontend 177/177, production build, backend package, assignment gate and temporary-database cleanup all pass. Final independent review, merge, release runbook and production acceptance remain outstanding; production is still `2.7.9 / c04e992b3840`, so TASK-213 is not yet merged or online.
