...
phase: maintenance
active_task: "TASK-168 done in production release 2.1.12."
next_action: "Ask user to retest AI 听记 and chat microphone from the browser; monitor /ws/asr access logs and ASR provider errors."
read_next:
  goals: false
  decisions: false
  issue_list: false
...
- TASK-168 fixed production AI 听记 and chat-window microphone voice recognition failures where `/ws/asr?token=...` received `401 Authentication required` before entering the ASR WebSocket handler.
- TASK-168 code fix: `TenantContextFilter` now lets only `/ws/asr` and `/ws/asr/*` reach the ASR WebSocket handler so the handler can validate query token; protected REST API authentication remains unchanged and covered by RBAC regression.
- TASK-168 verification passed: task authorization checks, TDD RED/GREEN `TenantContextFilterTest`, RBAC regression, backend compile, Compose config, release dry-run, ACR build/push, production backup/deploy, health smoke, and live `/ws/asr` query-token smoke that received `connected` and `started`.
- TASK-168 release: Git tag `2.1.12` pushed; backup `/opt/cici/backups/20260703-150359-before-2.1.12-asr-websocket-auth`; six production containers healthy; `/system/version` returns `version=2.1.12`, `imageTag=2.1.12`, `gitCommit=caf4baf90575`; public home `200`, unauthenticated `/auth/me` expected `401`.
