...
next_action: "TASK-171 CRM clean-embed hotfix 已本地验证；下一步发布 AgentCiCi 2.2.7、通过 cc-customization-expert-msapi 发布 CloudCC pagecomponent，并真实复测 CloudCC CRM 嵌入页不再显示 AgentCiCi 平台侧栏和 AI 应用列表。"
read_next:
  goals: false
  decisions: false
  issue_list: true
...
- Current branch: `main`; production is running release `2.2.6` from Git commit `3ed80e1873bf`; CRM clean-embed hotfix is implemented locally and awaiting `2.2.7` release.
...
- User reported the V7 CRM embedded page is functionally visible but visually wrong because it embeds the full AgentCiCi platform shell. The local hotfix adds `/app?aiApp=customer-workbench&embed=crm`, renders only `CustomerWorkbenchApp`, removes the pagecomponent outer header, and points CloudCC SSO `targetPath` to the clean embed route. Local Playwright DOM assertion passed with `hasRail=false`, `hasAiApps=false`, `hasEmbedded=true`; screenshot `.playwright-cli/page-2026-07-09T07-11-39-914Z.png`.
