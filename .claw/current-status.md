...
next_action: "用户在 2.8.68-dev.85e9ad5 页面分别复测阿里云对话听写和讯飞 AI 听记；随后回读后端首帧/总帧/字节与文字事件。"
read_next:
  goals: false
  decisions: false
  issue_list: true
...
- `TASK-352 / FEAT-062` 保持 `in_progress`：OACT 修复后，2026-09-02 HUMAN 复测两条入口都能进入录音态但没有文字；Nginx 回读阿里云/讯飞 WebSocket 均为 101，后端无厂商错误，断点后移到共享浏览器音频采集。`85e9ad51479e` 已进入本地 main 并部署为双制品 `2.8.68-dev.85e9ad5`：显式恢复 `AudioContext` 为 `running`，以 3 秒内第一帧 `onaudioprocess` 作为录音成功门禁；后端仅记录首帧、总帧数和字节数，不记录音频内容/身份/Token。聚焦前端 10 项、全量 `62 files / 346 tests`、production build、后端聚焦和 package 通过；两容器 healthy/restart=0，新 Chrome 授权态标签版本/console 0 通过，待 HUMAN 双入口复测回读音频帧和文字。
...
- `TASK-347 / FEAT-204` 已根据用户 2026-09-01 截图完成本地技术修复，进入 review：`629e35db6b89` 在 `mode=float` 渲染边界统一过滤全部内部 busy label，只保留无文字三点等待动效；SSE/Trace 与受信 `page` 嵌入阶段信息不变。聚焦 `8/8`、前端全量 `60 files / 334 tests`、production build 通过；frontend 从该本地 `main` 提交构建并运行 `2.8.68-dev.629e35d`，healthy/restart=0，backend 保持未改动的 `2.8.68-dev.1908000` 且 health UP。官网与 float Embed 200，部署 JS 指纹、浮窗 DOM、桌面截图、Nginx、结构化 5xx=0、console 0 通过；未在浏览器代 HUMAN 发送新消息，真实等待态目视接受仍待用户。UAT 仍为 `2.8.68-beta.3 / a5bbb114`，生产未修改。
...
- `TASK-333 / FEAT-203` 已补齐真实运行连接并更新本地开发环境，进入 HUMAN review：`demo-example.lifecycle` 及 r1 修订由 repeatable migration 持久化，保留测试域名与 Secret 引用名，状态诚实保持 `DRAFT / NOT_TESTED / activeRevisionId=null`；已发布 `initializationEngine=NONE` 版本不绑定该连接。代码提交 `40a27a2b` 已进入本地 `main`，后端聚焦/package、前端 7 项与全量 60/332、production build、域名/diff 门禁、PostgreSQL 16.9 空库 V128 专用迁移测试均通过。backend/frontend 运行 `2.8.68-dev.40a27a2 / 40a27a2b2983`、healthy/restart=0，其他八个容器未替换；数据库、Flyway、版本 API、HTTPS 200、匿名连接 API JSON 401 和最近 Nginx 5xx=0 通过。两类浏览器均无本地平台登录态，详情连接计数 1 与示例页 20 项参数待 HUMAN 目视；远程/UAT/生产未修改。
