...
- claimed_by: `codex`
- spec_path: `docs/specs/FEAT-197-agent-definition-external-identity.md`
- task_status_path: `.claw/tasks/TASK-322.md`
- blocked_by: `none`
- scope_files: `平台基础提示、Agent Definition 名称注入、统一运行时身份、定向测试、本地真实问候`
- next_action: `提交身份修复到本地 main，从该提交更新 cici.localhost backend 并验证产品经理问候。`
...
- completion_evidence: `4a697051；18 项模板聚焦回归、backend package、diff check 通过`
- next_action: `与 TASK-322 一并从本地 main 更新 cici.localhost backend，完成运行门禁。`
...
- completion_evidence: `1ad25d39/39424a98 已进远程 main；聚焦 12 项、全量 53 文件/294 项、build；本地 2.8.61-dev.1ad25d3 healthy；UAT 2.8.61-beta.30 技术门禁通过；登录态视觉验收待补`
- next_action: `平台用户登录 UAT 后打开组织切换弹层，复核长组织名称完整显示、hover/focus 和桌面视口边界。`
...
- blocked_by: `none`
- next_action: `已发布 2.8.61-beta.22；用新登记独立 Keycloak Client 完成真实登录、公司列表、公司上下文及 X-Company-Id 端到端业务验收，并由平台管理员复核受信应用目录与编辑弹窗`
...
- blocked_by: `none`
- next_action: 平台管理员配置真实业务空间凭据后完成连接检测、启用和 Agent 会话业务验收。
...
- next_action: `已随 2.8.60 发布生产；等待受权 PLATFORM_ADMIN 在正式页执行目标 Owner 身份协调并回读`
...
- assignment_path: `n/a`
- blocked_by: `none`
- next_action: 已完成受控单行更正与独立回读；未修改任何其他身份或业务数据。
...
- next_action: 已发布 `2.8.32 / 2e42ed3ec926` 并完成线上项目、需求、任务创建及回读闭环；后续更新/工时能力必须沿用确认门禁。
...
- next_action: 已发布 `2.8.31` 并以 `DAS-DEMO` 真实对话验收；后续可按同一工具扩展指定项目筛选。
...
- next_action: 已发布 `2.8.25 / 105cc666a958`；本地 PostgreSQL 恢复后可补跑账单集成测试。
...
- next_action: 已发布 `2.8.52` 统一密码与邀请落地修复；正常管理员邀请首个新成员时，应验证其完成 Keycloak 邮箱验证、设置密码和首次 OIDC 登录后才从 `PENDING_ACTIVATION` 转为 `ACTIVE`，不得使用非受权账号替代验收。
