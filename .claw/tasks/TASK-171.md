...
- 客户互动工作台客户列表错位热修生产发布验证 (2026-07-09) -> passed. 根因已验证：生产列表字段 `lastInteraction` 实际是互动摘要句子，前端却把它作为右侧 `time` 列渲染并在 `shortDate(...)` 解析失败后原样输出，叠加固定 92px 行高和可换行标签，导致 1620x812 下 `outsideCount=18`。修复后改用 `updatedAt` 渲染紧凑时间，把 `lastInteraction` 作为行内两行摘要，并稳定客户行网格与 badge/summary 边界。`git diff --check` passed；`npm run build` in `frontend/` passed（保留既有 Vite large chunk warning）；本地 5174 代理生产数据验证普通入口和 `embed=crm` 入口均 `outsideCount=0`、`rowOverlaps=[]`、`bodyOverflow=false`；提交 `d251a2661602` 已发布为生产版本 `2.2.11`；ECS 备份 `/opt/cici/backups/20260709-232920-before-2.2.11-task171-workbench-queue-layout`；backend/frontend 容器运行 `2.2.11`，六个服务 healthy；`/system/version` 返回 `version=2.2.11`、`imageTag=2.2.11`、`gitCommit=d251a2661602`；公网 `https://x.agentcici.com/`、`/app?aiApp=customer-workbench`、`/app?aiApp=customer-workbench&embed=crm` 和 production-IP resolved `https://onechat.agentcici.com/` 均返回 200；生产浏览器验证普通入口和 `embed=crm` 入口均 `outsideCount=0`、`rowOverlaps=[]`、`bodyOverflow=false`、`chatScrollbarVisible=false`。截图：`output/playwright/task171-prod-2.2.11-queue-layout.png`、`output/playwright/task171-prod-2.2.11-queue-layout-app.png`。
- Agent 平台内客户互动工作台视觉比例回归修复生产发布验证 (2026-07-09) -> passed. 提交 `8a003121df0c` 已发布为生产版本 `2.2.10`；ECS 备份 `/opt/cici/backups/20260709-225131-before-2.2.10-task171-workbench-visual-repair`；六个服务 healthy；`/system/version` 返回 `version=2.2.10`、`imageTag=2.2.10`、`gitCommit=8a003121df0c`；公网 `https://x.agentcici.com/`、`/app?aiApp=customer-workbench`、`/app?aiApp=customer-workbench&embed=crm` 和 production-IP resolved `https://onechat.agentcici.com/` 均返回 200；生产浏览器 1920x960 使用真实登录验证新客户模式 `versionText=2.2.10`、`heroCount=0`、`outerOverflow=false`、`brandVisible=false`、`verticalBadges=[]`、`chatScrollbarVisible=false`、`bottomVisible=true`，老客户模式验证 `hasExistingQueue=true`、`hasRiskPanel=true`、`outerOverflow=false`、`chatScrollbarVisible=false`、`bottomVisible=true`。截图：`output/playwright/task171-agent-prod-2.2.10-visual-repair.png`、`output/playwright/task171-agent-prod-2.2.10-existing-visual-repair.png`。
- Agent 平台内客户互动工作台视觉比例回归修复本地验证 (2026-07-09) -> passed. 针对用户反馈 `2.2.9` 视觉被压坏的问题，平台模式隐藏工作台内部重复品牌/面包屑，恢复三栏比例，压缩局部间距但保留设计稿结构，修复客户标签竖排，并将右侧 AI 客户助理对话区改为无可见滚动条模式。`git diff --check` passed；`npm run build` in `frontend/` passed（保留既有 Vite large chunk warning）；Playwright 1920x960 本地验证新客户模式 `heroCount=0`、`outerOverflow=false`、`brandVisible=false`、`verticalBadges=[]`、`chatScrollbarVisible=false`、workbench bottom visible；老客户模式验证 `hasExistingQueue=true`、`hasRiskPanel=true`、`outerOverflow=false`、`chatScrollbarVisible=false`、`bottomVisible=true`。截图：`output/playwright/task171-agent-workbench-repair-local.png`、`output/playwright/task171-agent-workbench-repair-existing-local.png`。
