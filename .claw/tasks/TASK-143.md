...
- 2026-05-31T21:15:00+08:00: aligned builder-seat accounting with the product permission model: active `OWNER` and `ORG_ADMIN` members now count as builder seats because they own organization management, configuration, and publishing authority. Updated FEAT-022/FEAT-037 specs and `AdminBillingIntegrationTest` expectations from `0 / 2` to `1 / 2`. Verification passed: `SPRING_DATASOURCE_URL='jdbc:postgresql://127.0.0.1:5432/agentcici_test' SPRING_DATASOURCE_USERNAME=cici SPRING_DATASOURCE_PASSWORD=cici123 SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE=3 mvn -q -Dtest='AdminBillingIntegrationTest' test`, `npm test -- AdminBillingPage.test.ts`, `npm run build`, and `git diff --check`. Existing Vite large chunk warning remains.
- 2026-05-31T19:35:00+08:00: addressed `/admin/billing` browser comments: admin-facing billing policy now shows `订阅`, the current edition summary hides internal deployment/status/code text, seat labels use `团队成员` and `构建者席位`, and quota rows explain used versus included seat counts. `npm test -- AdminBillingPage.test.ts`, `npm run build`, `git diff --check`, and desktop browser verification at `http://127.0.0.1:5173/admin/billing` all passed. Existing Vite large chunk warning remains. `.claw/test-report.md` was not edited because TASK-143 assignment does not authorize that path.
...
- 2026-05-31T08:55:00+08:00: fast-forwarded local `main` to `fa8df0e` and completed local MR validation: `git diff --check`, billing backend tests, billing frontend unit tests, and frontend production build all passed. Existing Vite large chunk warning remains.
