...
- AgentCiCi 当前只维护一条发布线。设下一次最终上线生产的目标版本为 `Q`，则本地开发版本为 `Q-dev.<7位Git SHA>`、UAT 候选为 `Q-beta.N`、正式生产为 `Q`；三者基础版本必须一致。
- 进入 UAT 后，后续修复仍属于 `Q`：本地 DEV 只随 `main` 更新提交指纹，UAT 只递增 `beta.N`。禁止出现 `R-dev.<SHA>` 晋级时变为更低的 `Q-beta.N`。
- 只有生产 `Q` 发布并通过发布后验证，才允许把本地版本基线与下一轮 UAT 目标共同推进到下一个生产版本 `R`。版本推进属于生产发布收尾动作，必须同步 `cc-local-stack/config/local-versions.env` 并记录提交。
...
UAT 发布前必须将脚本输出的 `Q-beta.N` 与 `cc-local-stack` 当前 AgentCiCi 基础版本 `Q` 比较；基础版本不同即停止发布。生产发布后再将两者共同推进，不得为了让命令通过而临时改写候选版本。
