...
- 前台 AI 应用区新增「知微画像」作为通用智能打标与画像引擎入口；保留既有「客户洞察」应用，不用重命名或覆盖原应用。
...
- 新增「知微画像」，副标题为「AI 智能打标与画像引擎」。
- 应用文案面向任意业务对象，强调多源事实、智能打标、画像归因、审核沉淀，不出现奔驰金融等外部品牌字样。
- 「知微画像」必须作为与「客户洞察」完全独立的 AI 应用落地：独立模块目录、独立渲染组件、独立页面信息架构，不复用客户洞察分段报告 UI。
- 「知微画像」按 CDP demo 结构做高保真还原，首屏包含对象列表、画像详情、标签库、AI 配置、运营看板五个工作区，并覆盖筛选表格、批量 AI 打标、标签证据、候选标签审核、策略阈值和运营指标。
- 原「客户洞察」作为独立 AI 应用保留，继续服务客户画像、合同订单、服务体验和一客一策场景。
...
- 前端验收：
  ...
  - 前台 AI 应用列表同时显示原「客户洞察」和新增「知微画像」；两者渲染路径和组件目录完全独立。
  - 「知微画像」展示通用智能打标画像语义，按 demo 高保真呈现对象列表、画像详情、标签库、AI 配置和运营看板，且不包含奔驰金融等外部品牌字样。
...
- 2026-07-06T16:16:00+08:00：已实现独立 `/admin/data-quality` 与 `/data-quality/*` 聚合 API；前台 AI 应用区新增「知微画像」，并保留原「客户洞察」入口。`npm run build` 与 Playwright 桌面验证通过，截图见 `output/playwright/zhiwei-portrait-ai-app.png`。
