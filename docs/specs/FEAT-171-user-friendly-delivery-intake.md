...
服务端只接受最近一份语法有效、未取消、无待澄清问题且 `ready_for_confirmation=true` 的标记。每次已创建或已取消事项会关闭此前草案，但不会终止整个会话；后续新事项仅从最近完成点之后的活动消息窗口恢复，支持同一会话连续创建项目、需求、缺陷或变更。`original_report` 和每条 `user_supplements` 必须逐字对应当前活动窗口中的真实用户消息，防止旧事项串入或模型改写原话。用户发送“确认提交需求/缺陷/变更”或“确认提交”后，服务端恢复草案并执行受控 Tool；没有有效草案时短确认不写入。
...
- 同一会话完成一个项目后可继续受理新缺陷，历史成功回执不会阻断后续草案，也不会被后续事项误用。
