...
related_decisions:
  ...
  - DEC-025
related_issues: none
updated_at: 2026-05-19T05:47:39Z
updated_by: ai
---
...
- 页面标题为「组织设置」。
- 主区域为 2 列：左侧资料表单，右侧只读摘要。
- 表单字段按「基本信息」「联系信息」「备注」分组。
- 底部固定在表单区域内的操作行：取消、保存。
...
3. 前端页面
   - 新增 `/admin/organization` 页面和导航。
   - 保存后刷新 AdminShell 组织名称展示。
   - 接入导出任务摘要。
...
- 管理端导航出现「组织设置」。
- 组织管理员进入 `/admin/organization` 后看到当前组织名称，而不是只看到 `orgId`。
- 组织名称可编辑，保存后刷新页面仍保持新名称。
- `orgId` 只读展示，可复制，不可编辑。
- 左侧导航身份区优先显示组织名称，辅助显示组织 ID。
- 组织资料字段保存成功后有明确反馈。
- 表单校验错误清晰，例如组织名称为空、邮箱格式错误、URL 格式错误。
- 最近数据导出记录可以查看，且创建导出任务仍走现有安全链路。
