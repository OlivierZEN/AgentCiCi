...
- 为证明真实执行而非计划文本，P2 固定创建两个步骤：`RETRIEVE` 记录既有知识检索决策与脱敏命中摘要，随后 `SYNTHESIZE` 记录既有模型生成结果。灰度期间向模型传入空工具定义，不执行任何工具；P3/P4 之前不允许模型自由生成计划或选择工具。
...
### 13.5 P6 实施契约：公司隔离灰度、运行指标与发布验证

- 三个现有开关保持服务器唯一事实源，并新增各自精确的 `allowed-company-ids` 白名单；只有开关开启、公司白名单和 Agent 白名单同时精确命中，才可进入 Plan-Exec、模式路由或 Reflect。白名单任一为空、公司/Agent 缺失或不命中，必须安全保留既有 Direct/ReAct，不创建运行或审查事实。客户端、会话、OpenAPI 凭据、提示词和 Trace 均不能选择、扩大或覆盖灰度范围。
- 所有 P6 代码默认关闭，不写入生产配置中的具体公司或 Agent；试点启用前必须由用户明确给出生产公司 ID、只读 Agent ID、启用层级和观察窗口。先仅启用 Plan-Exec，再在相同目标、无副作用事实与质量门通过后启用路由，Reflect 继续独立开关；不因发布自动开启任何开关。
- 运行时以低基数指标记录模式决定、Plan-Exec 启动/回退与 Reflect 的选中和终态，标签只允许 `mode`、`outcome`、`reason` 等固定枚举，严禁包含公司、Agent、会话、运行 ID、用户正文、凭据或错误原文。Trace/步骤计数一致性、跨公司拒绝、预算耗尽、失败/超时、确认等待和审查移交使用既有脱敏事实与运营查询复核，不产生新的业务写入。
- 发布前按 `docs/production-release-runbook.md` 完成定向回归、全新库迁移、编译、前端构建、Compose 渲染和 `scripts/release-acr.sh --dry-run`；真实生产发布、备份、镜像推送、迁移、试点开关、观察和回滚均使用同一版本号，并逐项记录。没有指定试点或生产发布授权时，P6 只交付默认关闭的代码、验证与可执行 Runbook 清单，不修改线上环境。
- 灰度失败、指标异常或无法确认边界时，优先关闭相应 P6 开关并保留 Direct/ReAct；不删除 `agent_task_*`/Trace 审计事实，不声称外部副作用已被应用回滚。
