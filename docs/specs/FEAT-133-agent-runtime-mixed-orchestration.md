...
### 13.5 P6 实施契约：组织隔离灰度、运行指标与发布验证

- 三个现有开关保持服务器唯一事实源，并新增各自精确的 `allowed-org-ids` 白名单；只有开关开启、组织白名单和 Agent 白名单同时精确命中，才可进入 Plan-Exec、模式路由或 Reflect。白名单任一为空、组织/Agent 缺失或不命中，必须安全保留既有 Direct/ReAct，不创建运行或审查事实。客户端、会话、OpenAPI 凭据、提示词和 Trace 均不能选择、扩大或覆盖灰度范围。
- 所有 P6 代码默认关闭，不写入生产配置中的具体组织或 Agent；试点启用前必须由用户明确给出生产组织 ID、只读 Agent ID、启用层级和观察窗口。先仅启用 Plan-Exec，再在相同目标、无副作用事实与质量门通过后启用路由，Reflect 继续独立开关；不因发布自动开启任何开关。
- 运行时以低基数指标记录模式决定、Plan-Exec 启动/回退与 Reflect 的选中和终态，标签只允许 `mode`、`outcome`、`reason` 等固定枚举，严禁包含组织、Agent、会话、运行 ID、用户正文、凭据或错误原文。Trace/步骤计数一致性、跨组织拒绝、预算耗尽、失败/超时、确认等待和审查移交使用既有脱敏事实与运营查询复核，不产生新的业务写入。
