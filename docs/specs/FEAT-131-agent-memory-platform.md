...
`TASK-229` 将可信服务端建立的通用主体上下文接入 Chat 编排器。它只能注入经关系库授权的摘要、结构化记忆与语义命中；客户端声明不构成组织、主体、Agent 或 scope 的信任依据。

`TASK-230` 将受认证 API 凭据固定映射到通用应用、主体类型、身份等级和允许命名空间。客户端只能传递外部主体标识，不能指定这些隔离或授权属性；绑定缺失按无记忆安全降级。
...
- `TASK-232` 的候选审核以候选写入时固化的 `agent_id` 作为权限归属，不能由 scope、请求参数或外部 metadata 推断。候选审核 API 必须同时校验组织、归属 Agent 和既有 Agent 权限。
