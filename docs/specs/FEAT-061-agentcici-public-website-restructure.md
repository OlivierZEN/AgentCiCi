...
- 三个公开套餐：标准版、专业版、企业版。
- 每个套餐必须展示月费、适用阶段、初始化 Credits、知识库容量、文档处理量、构建席位、团队成员上限、并发运行数和明确功能列表。
- 标准版建议价格：`¥1,999 / 月`，包含 `50,000 Credits / 月`、`10 GB 知识库容量`、`5,000 页文档处理 / 月`、`1 个构建席位`、`最多 20 个团队成员`、`2 路并发智能体运行`，覆盖首个 AutoService 或 AI 听记试点。
- 专业版建议价格：`¥6,999 / 月`，包含 `250,000 Credits / 月`、`100 GB 知识库容量`、`50,000 页文档处理 / 月`、`3 个构建席位`、`最多 100 个团队成员`、`10 路并发智能体运行`，覆盖 AutoService、AI 听记、客户洞察、Open API、SkillsHub 审核、运行记录和成本归因。
- 企业版建议价格：`¥18,800 / 月起`，包含 `1,000,000 Credits / 月`、`1 TB 起知识库容量`、`300,000 页文档处理 / 月起`、`8 个构建席位`、`团队成员按合同配置`、`50 路并发智能体运行起`，覆盖多智能体、多组织治理、SSO、高级审计、发布评测、预算预警、专属客户成功和 SLA。
- Credits 说明：Credits 按自然月发放，当月有效；超额 Credits 包从 `¥99 / 10,000 Credits` 起，年度客户可按阶梯价采购。
- 知识库容量说明：知识库容量包含原文存储、向量索引、元数据、日志和备份保留；超出套餐后容量包从 `¥99 / 100 GB / 月` 起。
- 文档处理说明：文档处理量覆盖解析、切分、清洗、向量化和重建索引；超出套餐后处理包从 `¥199 / 10,000 页` 起。
- 计费逻辑：Credits 负责智能体运行时真实消耗，知识库容量负责长期存储和索引资源，文档处理量负责批量导入与索引构建，并发运行数负责资源峰值，构建席位负责配置、发布和治理权限。普通团队成员不作为核心收费项，只作为权限、审计和组织管理容量。
- 补充包：Credits 包、知识库容量包、文档处理包、并发与构建扩展、上线服务。
