...
- 不重写历史聊天数据、Flyway 历史迁移或 Keycloak 密码/MFA 数据；本次前端修复只隔离浏览器内存状态，不删除任何公司的持久化会话。
...
8. **公司切换即失效**：任何由旧 `company_id` 取得的浏览器内存数据、正在进行的请求或实时流，都不得在新公司令牌生效后继续显示或写回页面。

## 前端公司切换会话隔离

### 问题与根因

工作台默认会话使用稳定的 `workbench:<agentKey>` 标识；该标识可在不同公司内分别存在。若浏览器缓存仅以 `agentKey` 或 `sessionId` 为 key，或者公司切换后未清空内存状态，则旧公司的消息可能继续渲染在新公司页面。后端的会话和消息查询已按当前 JWT `company_id` 过滤，但这不能替代前端状态隔离。

### 必须满足的行为

1. 公司切换成功、OIDC 完成登录或 CloudCC SSO 完成登录，只要 `companyId` 发生变化，前端必须立即清空旧公司的会话列表、消息缓存、活动会话、工作台消息、工作台会话选择、运行态、知识库选择、技能与快捷指令缓存。
2. 缓存键必须包含 `companyId`：普通会话至少为 `companyId + sessionId`，工作台至少为 `companyId + agentKey`。不得依赖跨公司可碰撞的默认工作台会话 ID 作为唯一浏览器缓存键。
3. 所有会话列表、消息、工作台、智能体、运行日志、技能与快捷指令异步响应必须在写入 React 状态前确认其发起时的 `companyId` 与当前认证公司一致；旧响应静默丢弃。
4. 公司切换时必须终止旧公司的 SSE/流式聊天请求；即使旧流在取消后晚到，也不得修改新公司页面。
5. 页面只显示当前公司通过当前令牌读取到的会话。切换后的空态是允许结果，绝不可用旧公司内容填充。
6. 认证后 SPA 的入口 HTML（含 `/app` 回退）必须禁止浏览器复用旧版本；只有带内容哈希的 `/assets/` 工件可以长期 immutable 缓存。否则已修复的租户隔离逻辑无法抵达用户浏览器，旧工件仍可能按旧键显示跨公司内容。
7. 会话 SSE 与定时刷新必须在建立连接时捕获公司作用域版本；回调开始、异步列表读取后、继续读取消息前均应复核该版本。旧作用域回调只能静默退出，不能借用新令牌请求或更新新公司的页面。

### 验收标准

- 同一账户在两个公司中各有同名 `workbench:cici` 会话时，从公司 A 切至公司 B 后，只显示公司 B 的消息；不得短暂保留公司 A 消息。
...
- 发布后普通刷新 `/app` 必须取得当前入口 HTML 和当前版本的哈希 JS；缓存命中旧入口不应再成为数据隔离补丁未生效的原因。
